...
object TestRunner {
  def renderResults(results: Seq[(String, HTree[String, Result])],
                    ...
                    failureHeader: String = DefaultFormatters.failureHeader): (utest.fansi.Str, Int, Int) = {
    ...
    val formatted = DefaultFormatters.formatSummary(
      ...
      body = {
        val frags = for {
          ...
          frag <- Seq[utest.fansi.Str]("\n", str)
        } yield frag
        utest.fansi.Str.join(frags.drop(1))
      },
      failureMsg = {
        val frags = for{
          f <- failures
