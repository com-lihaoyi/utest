...
trait Formatter {
  ...
  def exceptionMsgColor = toggledColor(utest.shaded.fansi.Attrs.Empty)
  ...
  def exceptionPunctuationColor = toggledColor(utest.shaded.fansi.Attrs.Empty)
  ...
  def formatException(x: Throwable, leftIndent: String) = {
    val output = mutable.Buffer.empty[utest.shaded.fansi.Str]
    var current = x
    while(current != null){
          ...
          lineWrapInput(
            current match{
              case colored: ColorMessageError =>
                utest.shaded.fansi.Str.join(Seq(exCls, ": ", colored.coloredMessage))
              case _ =>
                current.getMessage match{
                  case null => exCls
                  case nonNull => utest.shaded.fansi.Str.join(Seq(exCls, ": ", exceptionMsgColor(nonNull)))
                }
            }
            ,
            leftIndent
      ...
      StackMarker.filterCallStack(stack)
        .foreach { e =>
          ...
          val filenameFrag: utest.shaded.fansi.Str = e.getFileName match{
          ...
          val wrapper =
            if(exceptionStackFrameHighlighter(e)) utest.shaded.fansi.Attrs.Empty
            else utest.shaded.fansi.Bold.Faint
    ...
    utest.shaded.fansi.Str.join(output.toSeq)
  }

  def lineWrapInput(input: utest.shaded.fansi.Str, leftIndent: String): Seq[utest.shaded.fansi.Str] = {
    val output = mutable.Buffer.empty[utest.shaded.fansi.Str]
  ...
  def joinLineStr(lines: Seq[utest.shaded.fansi.Str], leftIndent: String) = {
    utest.shaded.fansi.Str.join(lines.flatMap(Seq[utest.shaded.fansi.Str]("\n", leftIndent, _)).drop(2))
  }

  private[this] def prettyTruncate(r: Result, leftIndent: String): utest.shaded.fansi.Str = {
    r.value match{
      case Success(()) => ""
      case Success(v) =>
  ...
  def wrapLabel(leftIndentCount: Int, r: Result, label: String): utest.shaded.fansi.Str = {
    val leftIndent = "  " * leftIndentCount
    val lhs = utest.shaded.fansi.Str.join(Seq(
      leftIndent,
  ...
  def formatSingle(path: Seq[String], r: Result): Option[utest.shaded.fansi.Str] = Some{
    wrapLabel(0, r, path.mkString("."))
  }

  def formatIcon(success: Boolean): utest.shaded.fansi.Str = {
    formatResultColor(success)(if (success) "+" else "X")
  }

  def formatSummary(topLevelName: String, results: HTree[String, Result]): Option[utest.shaded.fansi.Str] = Some{
    ...
    val (rendered, totalTime) = rec(0, relabelled){
      case (depth, Left((name, millis))) =>
        utest.shaded.fansi.Str("  " * depth + "- " + name + " ") ++ formatMillisColor(millis + "ms")
  ...
  private[this] def rec(depth: Int, r: HTree[String, Result])
                       (f: (Int, Either[(String, Long), Result]) => utest.shaded.fansi.Str): (Seq[utest.shaded.fansi.Str], Long) = {
    r match{
