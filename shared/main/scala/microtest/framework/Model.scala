 ...
 * comes contextually when traversing the [[TestTreeSeq]] to reach it.
 */
...
class TestTreeSeq(tests: Tree[Test]) {
  def runAsync(onComplete: (Seq[String], Result) => Unit,
              ...
              (implicit ec: ExecutionContext): Future[Tree[Result]] = Future{
    ...
    Future.sequence(childRuns).map{ results =>
  ...
  def run(onComplete: (Seq[String], Result) => Unit = (_, _) => (),
    ...
    Await.result(
      current.runAsync(onComplete, indices, strPath),
      10.hours
    )
  }
}
